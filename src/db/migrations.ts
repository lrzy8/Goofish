...
function safeAddColumn(table: string, column: string, type: string) {
  try {
    ...
    `)
    const tableInfo = checkColumn.get(table) as { sql: string } | undefined
    
    if (tableInfo) {
      const createTableSql = tableInfo.sql.toLowerCase()
      // 检查列名是否已经存在于表结构中
      if (createTableSql.includes(` ${column.toLowerCase()} `)) {
        logger.debug(`列 ${column} 已存在于表 ${table} 中，跳过添加`)
        return
      }
    }
    ...
    logger.debug(`已成功添加列 ${column} 到表 ${table}`)
  } catch (error) {
    logger.warn(`添加列 ${column} 到表 ${table} 时出错: ${error}`)
    // 列可能已存在或其他错误，忽略
  }
}
